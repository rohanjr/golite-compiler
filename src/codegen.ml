...
and tp_of_primaryexpr (pe : primaryexpr) : tp = match pe with
  ...
  | FunApp (_, _, _, tpor) | Append (_, _, _, tpor) | Cast (_, _, _, tpor) ->
      begin match !tpor with
      | None -> raise (InternalError ("Primary expression " ^ pretty_primaryexpr pe ^ " has no type during code generation."))
      | Some t -> t
      end
...
and gen_arrayaccess (pe : primaryexpr) (e : expr) : string =
  ...
  | t -> raise (InternalError ("Primary expression " ^ pretty_primaryexpr pe ^ " has type " ^ pretty_tp t ^
